...
bool KeyboardApplication::winEventFilter(MSG * msg, long * result)
{
  if (m_keyWidget)
  {
    HWND keyWnd = (HWND)m_keyWidget->winId();
    HWND forWnd = GetForegroundWindow();
    if( IsWindow(forWnd) ) {
      if( forWnd != keyWnd ) {
        if((HWND)m_focusWindow != forWnd){
           if( IsWindow((HWND)m_focusWindow) ){
                        AttachThreadInput(
                                GetWindowThreadProcessId(keyWnd,NULL),
                                GetWindowThreadProcessId((HWND)m_focusWindow,NULL),
                                FALSE);
           }
              m_focusWindow =(WId)forWnd;
              AttachThreadInput(
                            GetWindowThreadProcessId(keyWnd,NULL),
                            GetWindowThreadProcessId((HWND)m_focusWindow,NULL),
                            TRUE);
  ...
  default:
     return KeyboardApplication::winEventFilter(msg,result);
  }// switch
}//winEventFilter
...
void KeyboardApplication::clearFocus()
{
    if(IsWindow((HWND)(m_focusWindow))) {
       ...
       if(IsWindow(wnd)) {
          if(wnd == (HWND)m_focusWindow)
             { return; }
       }

       SetForegroundWindow((HWND)m_focusWindow);
       SetFocus((HWND)m_focusWindow);
    }
}
